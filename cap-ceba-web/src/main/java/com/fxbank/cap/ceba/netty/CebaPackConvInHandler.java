...
public class CebaPackConvInHandler extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		try {
			...
			if(!strMsg.toString().contains("BJCEBRWKRes")&&!strMsg.toString().contains("BJCEBRWKReq")) {
				...
				// 校验MAC		
				if(!workKeyService.genMac(myLog, strMsg).equals(mac)) {
			...
			if (strMsg.contains("<in>")) {
				...
				if (txCode.equals("Error")) {
					...
					String channel = "CEBA-"+repError.getHead().getTrmSeqNum();
					jedisPublish(myLog, channel.getBytes(), strMsg.getBytes(ServerInitializer.CODING));
					...
					String channel = "CEBA-"+dtoBase.getHead().getTrmSeqNum();
					myLog.info(logger, "报文同步通道编号=[" + channel + "]");
					jedisPublish(myLog, channel.getBytes(), strMsg.getBytes(ServerInitializer.CODING));
				}
			}
	...
	public void jedisPublish(MyLog myLog, byte[] key, byte[] value) {
		...
		try (Jedis jedis = myJedis.connect()) {
			String stimeout = jedis.get(COMMON_PREFIX+"timeout");
			if (stimeout == null) {
				timeout = 60;
			} else {
				try {
					...
					myLog.error(logger, "同步等待超时时间配置异常，取默认值60");
					timeout = 60;
				}
			}
