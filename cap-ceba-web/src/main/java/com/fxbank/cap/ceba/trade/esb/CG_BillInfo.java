...
public class CG_BillInfo extends BaseTradeT1 implements TradeExecutionStrategy {
	...
	*/
	@Override
	public MODEL_BASE othCharge(DataTransObject dto) throws SysTradeExecuteException {
		REQ_30062001001 reqDto = (REQ_30062001001) dto;
		MyLog myLog = logPool.get();
		REQ_30062001001.REQ_BODY reqBody = reqDto.getReqBody();
		...
		//客户姓名非必填
		String companyListCustomer = null;
		//合同号必填
		String companyListContract = null;
		//备用字段filed1必填
		String companyListFiled1 = null;
		try (Jedis jedis = myJedis.connect()) {
			...
			companyListCustomer = jedis.get(COMMON_PREFIX + "company_list_customer");
			companyListContract = jedis.get(COMMON_PREFIX + "company_list_contract");
			companyListFiled1 =  jedis.get(COMMON_PREFIX + "company_list_filed1");
		}
		...
		JSONObject companyListCustomerJson= JSONObject.parseObject(companyListCustomer);
		JSONObject companyListContractJson= JSONObject.parseObject(companyListContract);
		JSONObject companyListFiled1Json= JSONObject.parseObject(companyListFiled1);
		if(null==companyListCustomerJson.get(companyId)) {
		...
		if(null!=companyListContractJson.get(companyId)) {
		tin.setContractNo(reqBody.getContractNo());
		}
		CebaBillInfoLogModel record = cebaBillInfoLogService.getBillInfoBySeqNo(myLog,reqDto.getReqSysHead().getSeqNo());
		if(null!=companyListFiled3Json.get(companyId)) {
		tin.setFiled3(reqBody.getPyCityCode());
		}
		if(null!=companyListFiled3sJson.get(companyId)) {
		     req.getTin().setFiled3("410");
		}
		if(null!=companyListFiled1Json.get(companyId)) {
			tin.setFiled1(record.getItem1());
		}
		REP_BJCEBBCRes res = (REP_BJCEBBCRes) forwardToCebaService.sendToCeba(req);
		    return res;
	}
	...
	*/
	@Override
	public void updateOthSuccess(DataTransObject dto, MODEL_BASE model) throws SysTradeExecuteException {
		REQ_30062001001 reqDto = (REQ_30062001001) dto;
		MyLog myLog = logPool.get();
	...
	*/
	@Override
	public DataTransObject backMsg(DataTransObject dto,MODEL_BASE model) throws SysTradeExecuteException {
		REQ_30062001001 reqDto = (REQ_30062001001) dto;
	...
	*/
	...
	/** 
	* @Title: queryOth 
	* @Description: 查询缴费单信息（光大银行记账超时不查询）
	* @param @param dto
	* @param @return
	* @param @throws SysTradeExecuteException    设定文件 
	* @throws 
