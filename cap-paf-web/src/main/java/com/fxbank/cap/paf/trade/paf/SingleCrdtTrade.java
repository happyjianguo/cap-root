...
public class SingleCrdtTrade implements TradeExecutionStrategy {
	...
	public DataTransObject execute(DataTransObject dto) throws SysTradeExecuteException {
		...
		if (!deAccountName.equals(reqDto.getBody().get(DEACCTNAME).toString())) {
			PafTradeExecuteException e = new PafTradeExecuteException(PafTradeExecuteException.PAF_E_10007);
		...
		if ("1".equals(settleType)) {
			...
			if (!crAccountName.equals(reqDto.getBody().get(CRACCTNAME).toString())) {
				...
				PafTradeExecuteException e = new PafTradeExecuteException(PafTradeExecuteException.PAF_E_10008);
				myLog.error(logger, e.getRspCode() + " | " + e.getRspMsg());
				throw e;
			}
			try {
				...
				myLog.error(logger, "核心系统记账失败,付款号" + reqDto.getBody().get(DEACCTNO).toString() + "收款号"
						+ reqDto.getBody().get(CRACCTNO).toString());
				myLog.error(logger, e.getRspCode() + " | " + e.getRspMsg());
				PafTradeExecuteException e1 = new PafTradeExecuteException(PafTradeExecuteException.PAF_E_10003,
						e.getRspMsg());
			...
			PafTradeExecuteException e = new PafTradeExecuteException(PafTradeExecuteException.PAF_E_10002);
			myLog.error(logger, e.getRspCode() + " | " + e.getRspMsg());
			throw e;
		}
		// 银行主机处理状态
	 ...
	 * dto @param @return @param @throws SysTradeExecuteException 设定文件 @return
	 * ESB_REP_30011000101 返回类型 @throws
	 */
	private ESB_REP_30011000101 innerCapCharge(SER_REQ_DATA reqDto) throws SysTradeExecuteException {
		...
		ESB_REP_30011000101 esbRep_30011000101 = IForwardToESBService.sendToESB(esbReq_30011000101, reqBody_30011000101,
	...
	private ESB_REP_30041000406 outerCapCharge(SER_REQ_DATA reqDto) throws SysTradeExecuteException {
		...
		ESB_REQ_30041000406 esbReq_30041000406 = new ESB_REQ_30041000406(myLog, reqDto.getSysDate(),
				reqDto.getSysTime(), reqDto.getSysTraceno());
		ESB_REQ_SYS_HEAD reqSysHead = new EsbReqHeaderBuilder(esbReq_30041000406.getReqSysHead(), reqDto)
				.setBranchId(txBrno).setUserId(txTel).build();
		...
		ESB_REP_30041000406 esbRep_30041000406 = IForwardToESBService.sendToESB(esbReq_30041000406, reqBody_30041000406,
				ESB_REP_30041000406.class);
		return esbRep_30041000406;
	}

	/**
	 * @Title: initRecord @Description: 登记单笔付款交易信息 @param @param
	 * reqDto @param @return @param @throws SysTradeExecuteException 设定文件 @return
	 * SinglCrdtInfoModel 返回类型 @throws
	 */
	private SinglCrdtInfoModel initRecord(SER_REQ_DATA reqDto) throws SysTradeExecuteException {
		...
		SinglCrdtInfoModel record = new SinglCrdtInfoModel(myLog, reqDto.getSysDate(), reqDto.getSysTime(),
	...
	/**
	 * @Title: updateCapRecord @Description: 更新本金记账信息 @param @param
	 * reqDto @param @param txStatus 交易状态 @param @param capCode 本金记账状态 @param @param
	 * capMsg 本金记账状态信息 @param @param capSeqno 本金记账流水号 @param @return @param @throws
	 * SysTradeExecuteException 设定文件 @return SinglCrdtCapUpdModel 返回类型 @throws
	 */
	private SinglCrdtCapUpdModel updateCapRecord(SER_REQ_DATA reqDto, String txStatus, String capCode, String capMsg,
			String capSeqno) throws SysTradeExecuteException {
		MyLog myLog = logPool.get();
		SinglCrdtCapUpdModel record = new SinglCrdtCapUpdModel(myLog, reqDto.getSysDate(), reqDto.getSysTime(),
				reqDto.getSysTraceno());
	 ...
	 * reqDto @param @return @param @throws SysTradeExecuteException 设定文件 @return
	 * ESB_REP_30013000510 返回类型 @throws
	 */
	private ESB_REP_30013000510 queryDeAccount(SER_REQ_DATA reqDto) throws SysTradeExecuteException {
		...
		ESB_REQ_30013000510 esbReq_30013000510 = new ESB_REQ_30013000510(myLog, reqDto.getSysDate(),
				reqDto.getSysTime(), reqDto.getSysTraceno());
		ESB_REQ_SYS_HEAD reqSysHead = new EsbReqHeaderBuilder(esbReq_30013000510.getReqSysHead(), reqDto)
				.setBranchId(txBrno).setUserId(txTel).build();
		...
		ESB_REP_30013000510 esb_rep_30013000510 = IForwardToESBService.sendToESB(esbReq_30013000510,
				reqBody_30013000510, ESB_REP_30013000510.class);
